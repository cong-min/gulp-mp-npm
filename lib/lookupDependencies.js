...
function lookupDependencies(filepath, fileContent, options, deps = {}, root = true) {
    ...
    Object.keys(deps).forEach((depFilepath)=> {
      cacheKey += `_${depFilepath}`;
    })
    if(dependenciesCache.has(cacheKey)) {
      return dependenciesCache.get(cacheKey);
    }
    // 分析并解析依赖映射
    ...
    dependenciesCache.set(cacheKey, deps);
    return deps;
}
