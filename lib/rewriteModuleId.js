...
module.exports = (file, pkgList, npmDirname = defaultNpmDirname) => {
    ...
    function rewriteRule(moduleId) {
        ...
        const rewritable = (npmDirname === defaultNpmDirname && isCss) // 官方方案中所有 wxss 引入其他文件需要重写
            ...
            || (npmDirname !== defaultNpmDirname && pkgName && pkgList[pkgName]);

        if (rewritable) {
            if (pkgName && pkgList[pkgName]) { // pkg 存在
                const pkg = pkgList[pkgName];
                let pkgPath = pkg.path;
                const pkgPathSplit = pkgPath.replace(/\\/g, '/').split('/node_modules/');
                const pkgBase = npmDirname === defaultNpmDirname
                    ? pkgPathSplit[0]
                    // 如果非官方方案, 则无多层依赖结构
                    : pkgPathSplit.slice(0, pkgPathSplit.length - 1).join('/node_modules/');
                // 如果引入的入口文件，则修改路径为包入口文件 main
                if (moduleId === pkgName) {
                    if (npmDirname === defaultNpmDirname) { // 官方方案，入口为追加 index
                        pkgPath = path.join(pkgPath, `index${path.extname(pkg.main) || '.js'}`);
                    } else if (pkg.isMiniprogramPkg) { // 自定义提取方案时，小程序专用包入口替换 buildPath
            ...
            } else if (!pkgName) { // 引用的不是 pkg
                const targetPath = path.join(path.dirname(file.originPath || file.path), moduleId);
                // 找出 targetPath 在文件下的相对路径
                let relativePath = path.relative(
                    replaceNodeModulesPath(path.dirname(file.path), npmDirname),
